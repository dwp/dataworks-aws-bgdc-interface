resource "aws_s3_bucket_object" "metadata_script" {
  ...
  key        = "component/${local.component}/metadata.sh"
...
resource "aws_s3_bucket_object" "emr_setup_sh" {
  bucket = data.terraform_remote_state.common.outputs.config_bucket.id
  key    = "component/${local.component}/emr-setup.sh"
  content = templatefile("${path.module}/bootstrap_actions/emr-setup.sh",
    {
      ...
      acm_cert_arn                    = aws_acm_certificate.bgdc_interface.arn
      ...
      cwa_log_group_name              = aws_cloudwatch_log_group.bgdc.name
      ...
      cwa_bootstrap_loggrp_name       = aws_cloudwatch_log_group.bgdc_cw_bootstrap_loggroup.name
      cwa_steps_loggrp_name           = aws_cloudwatch_log_group.bgdc_cw_steps_loggroup.name
      cwa_yarnspark_loggrp_name       = aws_cloudwatch_log_group.bgdc_cw_yarnspark_loggroup.name
  })
}

resource "aws_s3_bucket_object" "ssm_script" {
  bucket  = data.terraform_remote_state.common.outputs.config_bucket.id
  key     = "component/${local.component}/start_ssm.sh"
  content = file("${path.module}/bootstrap_actions/start_ssm.sh")
}

resource "aws_s3_bucket_object" "installer_sh" {
  bucket = data.terraform_remote_state.common.outputs.config_bucket.id
  key    = "component/${local.component}/installer.sh"
...
resource "aws_s3_bucket_object" "logging_script" {
  bucket  = data.terraform_remote_state.common.outputs.config_bucket.id
  key     = "component/${local.component}/logging.sh"
...
resource "aws_s3_bucket_object" "cloudwatch_sh" {
  bucket = data.terraform_remote_state.common.outputs.config_bucket.id
  key    = "component/${local.component}/cloudwatch.sh"
